...
const EditTask = () => {
    ...
    // Fetch task and populate form
    const capitalize = (str) =>
        str ? str.charAt(0).toUpperCase() + str.slice(1).toLowerCase() : 'Medium';

    useEffect(() => {
        const fetchTask = async () => {
            try {
                const res = await api.get(`/app/tasks/${id}`);
                const task = res.data.task;
                setForm({
                    title: task.title || '',
                    description: task.description || '',
                    priority: capitalize(task.priority), // ← 'high' → 'High'
                    date: task.due_date || '',
                });
            } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            await api.put(`/app/tasks/${id}/update`, form);
            setSubmitted(true);
            setTimeout(() => navigate('/app/tasks'), 1800);
        } catch (error) {
            toast.error(error.response?.data?.message || 'Task update failed.');
        }
    };

    const cfg = priorityConfig[form.priority] ?? priorityConfig['Medium'];

    if (loading) {
        return (
            <div className="min-h-screen bg-[#12122a] flex items-center justify-center px-4">
                <div className="flex flex-col items-center gap-4">
                    <div className="w-10 h-10 rounded-full border-2 border-[#7c6ef0] border-t-transparent animate-spin" />
                    <p className="text-[#6a6a9a] text-sm">Loading task...</p>
                </div>
            </div>
        );
    }
        ...
        return (
            <div className="min-h-screen bg-[#12122a] flex items-center justify-center px-4">
                <div className="flex flex-col items-center gap-4 animate-pulse">
                    ...
                    <h2 className="text-white text-2xl font-bold">Task Updated!</h2>
            ...
            <div className="relative z-10 max-w-xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-white text-2xl font-black tracking-wide">Edit Task</h1>
                            <p className="text-[#6a6a9a] text-xs">Update the details below</p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-[#1e1e40] border border-[#2e2e5a] rounded-2xl p-6 flex flex-col gap-6">
                        ...
                        <div className="flex flex-col gap-2">
                            ...
                            <input
                                type="date"
                                name="date"
                                value={new Date(new Date(form.date).getTime() - new Date(form.date).getTimezoneOffset() * 60000)
                                    .toISOString()
                                    .split("T")[0]}
                                onChange={handleChange}
                                className={`w-full bg-[#12122a] border rounded-xl px-4 py-3 text-white text-sm focus:outline-none transition-all duration-200 [color-scheme:dark]
